    ...
    () => [
      {
        ...
        render: (_, { name, lastname }) => (
          <div>
            {firstCharUppercase(name)}&nbsp;{firstCharUppercase(lastname)}
          </div>
        ),
      },
      {
        ...
        dataIndex: "age",
        key: "id",
        render: (_, { year, month, day }) => (
          <div>{getAge(`${year}/${month}/${day}`)}</div>
        ),
        sorter: (a, b) =>
          getAge(`${a.year}/${a.month}/${a.day}`) -
          getAge(`${b.year}/${b.month}/${b.day}`),
